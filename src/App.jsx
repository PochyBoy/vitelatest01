...
function App() {
  ...
  let tabla = ciudad.map(e=> <tr> <th>{e.municipi}</th><th>{e.poblacio}</th><th>{e.provincia}</th><th>{e.comarca}</th></tr>)

    function ordena(c1,c2){
      if(c1.poblacio<c2.poblacio){
        return -1
      }else if(c1.poblacio>c2.poblacio){
        return -1
      }else{
        return 0
      }
    }
    let poblacionTotal = ciudad.sort(ordena)
//como mostrar en la pantalla al ser presionado?
    let poblacion_Total = poblacionTotal.map (e => <tr> <th>{e.municipi}</th><th>{e.poblacio}</th><th>{e.provincia}</th><th>{e.comarca}</th></tr>)
  ...
  return (
    ...
    <table>{tabla}</table>
      <h1>Holi</h1>
    <button type="tex" onClick={poblacion_Total} >poblacio</button>
